...
if(isset($_POST['update_pass']))
{
    ...
    if(empty($op) || empty($np) || $np !== $c_np)
    {
        ...
        echo "<script>window.location = '/AGCOWC PROJECTJS/change.php'</script>";
        exit();
    }
    ...
    try {
        ...
        if ($query_execute == -1) {
            ...
            echo "<script>window.location ='/AGCOWC PROJECTJS/home.php'</script>";
            exit();
        } else {
            $_SESSION['message'] = "Not updated";
            header('Location: /AGCOWC PROJECTJS/change.php');
            exit(0);
        }
